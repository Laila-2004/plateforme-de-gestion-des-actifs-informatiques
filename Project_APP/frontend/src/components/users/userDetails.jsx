...
const UserDetails = ({ isOpen, onClose, user, computers, phones, printers, ecrants,onEdit }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full p-8 relative">
          ...
          {assignedMaterials.length > 0 ? (
            <ul className="list-disc pl-6 space-y-2">
              {assignedMaterials.map((material) => (
                <li key={material.id}>
                  <strong className="text-indigo-600">{material.type}</strong> : nome :  <span className="text-gray-700">{material.name}</span> - marque : <span className="text-gray-700">{material.marque}</span> - etat :<span className="text-gray-700">État : {material.etat}</span>
                </li>
              ))}
            </ul>
          ) : (
        ...
        <button className="mt-6 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={() => onEdit(user)}>Modifier</button>
      </div>
    </div>
  );
};
